...
useEffect(()=>{
   const fetchdata=async()=>{
      // await axios.get('https://bank-server-1.onrender.com/data').then((item)=>{setData(item.data)})
      await axios.get('https://bank-server-opxj.onrender.com/data').then((item)=>{setData(item.data)})
   };fetchdata()
},[]);
...
function handleClick(e){
   ...
   for(let i=0;i<data.length;i++){
      if(data[i].id === Number(userId)){
         ...
         // let url=`https://bank-server-1.onrender.com/update/${data[i]._id}`
         let url=`https://bank-server-opxj.onrender.com/update/${data[i]._id}`
